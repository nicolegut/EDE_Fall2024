...
```{r}
#Exercise 1:
dens.ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(aes(color = as.factor(Year))) +
  scale_x_continuous(expand = c(0, 0)) + 
  scale_y_continuous(expand = c(0, 0))
print(dens.ozone.plot)
...
box.ozone.plot <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone))+
  geom_boxplot()+
...
pm.ozone.reg <- 
  lm(PM2.5~Ozone, EPAair)
pm.ozone.reg
summary(pm.ozone.reg)
...
EPAcorr <- cor(EPAair.subset)
corrplot(EPAcorr, upper = "ellipse")
...
ozoneall.reg <- lm(data = EPAair,
                   PM2.5~Ozone+Year+Month+mean_Lng+mean_Lat)
summary(ozoneall.reg)
