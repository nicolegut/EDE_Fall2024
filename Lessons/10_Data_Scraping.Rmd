...
#paste0 elimnates space between elements we cant to concatenate
...
scrape.it <- function(the_year, the_facility){
  ...
  #iterating --> might want slower scraping speed to not have webpages think they are under attack
...
```{r test.the.function}
#Run the function
the_df <- scrape.it(2010,'0004-0001')
view(the_df)
...
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it,
                  the_facility=my_facility) #repeating the function for each value in the array, while specifying the second value in the function, has the second one be static

#OR, use purrr's map function
the_dfs <- map(the_years,scrape.it,the_facility=my_facility)
#the years, the function, the facility/ second value
...
the_facility_ids <- the_main_website %>% 
  html_nodes('#content a') %>% #html_text()   #Extract a list of html nodes taged with "content a"
...
#Subset the facilities - first 20
...
#iterates through lists in parallel - lists must be same size!
...
#random grab of 3 sites and then designate 3 years
...
#to do 2 element lists --> 3 diff sites 3 diff years --> list of 9 values
#generates a list of unique combinations?
