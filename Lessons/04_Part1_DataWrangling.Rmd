...
`==` comparing values to specific value
`!=` checking if values are not equal
`<`  
`<=`
`>` 
`>=`
`&` probability --> if you want both to be true
`|` probability --> if one or the other is true

```{r filtering data}
class(NTL.phys.data$lakeid)
class(NTL.phys.data$depth)

# matrix filtering --> filtering rows that have a depth of 0
...
NTL.phys.data.year1 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")
summary(NTL.phys.data.year1)

```
Question: Why don't we filter using row numbers?

> Answer: If row numbers shift with further organization or changes to the dataset, you may not be analyzing the data you intended. 
...
NTL.temp.desc <- arrange(NTL.phys.data, desc(temperature_C))
...
```{r selecting columns}
#sampledate:temperature_C --> ( : ) selects all columns between
...
answer > creates cleaner r scripts, keeps them as neat and simple as possible
