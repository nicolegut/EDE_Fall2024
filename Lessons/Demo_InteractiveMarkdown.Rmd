...
my_plot <- nutrient_data %>% 
  ...
  scale_x_date(
    date_breaks = "1 years",
    date_labels = '%Y') +
  scale_shape_manual(values = c(17, 15)) + #sets the shapes to square (15)/ triangle (17)
  labs(
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = .data[[the_parameter]],
    #y = !!sym(the_parameter), <-- outdated version
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
