...
NTL.nutrient.data <- read.csv(
  "./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv",
...
#need to change sample date to a date format
...
#drop levels for all lakes that have no values
...
#factor because used strings as factors command
#y because only 2 numbers
...
NTL.nutrient.data.PeterPaul <- 
  ...
  mutate(month = month(sampledate)) %>% # mutate to create month column
  select(lakeid:daynum, month, sampledate:comments) %>% # put month in right spot
  drop_na(depth) #dropping all na's

summary(NTL.nutrient.data.PeterPaul$depth)
...
##only subsetting values that are not na --> not removing na values, stores original data for later
...
# Spread nutrient data into separate columns <-- might not work if one row has na's on all four nutrient columns
...
#ex. inserting the month column like previously
...
#only adding columns, not more rows?
