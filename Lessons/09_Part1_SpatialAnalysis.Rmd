...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(                            #convert to spatial data fram
...
mapview(EPAair.sf, cex = 'mean_PM25') #cex = size
...
mapview(sfg_GraingerHall)     #will just visualize the point -> doesn't know where point is, will not assume where coords are

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #conv to simple feature column
class(sfc_GraingerHall)
mapview(sfc_GraingerHall) 

#Upgrade to a spatial dataframe, add attributes
...
#View the result
mapview(the_buffer) + mapview(sf_GraingerHall, color = "green")
```
