...
huc8_sf2 <- st_read(here('Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == 'Upper Neuse')

#Create a map
mapview(huc8_sf, col.region = "grey")+
  mapview(huc8_sf2, col.region = "yellow", color = "blue")

ggplot()+
  geom_sf(data = huc8_sf, fill = "grey")+
  geom_sf(data = huc8_sf2, fill = "yellow", color = "blue")
...
#left join drops all values in svi2020_nc_raw that don't have a match in counties_sf
...
wind_df <- read.csv(
  ...
  mutate(HUC_8 = substr(huc_nc_raw$HUC_12, 1,8)) %>% 
  group_by(HUC_8) %>% 
  summarise(
    wind = sum(AvgWindEnergy)
  )
...
#View the outputs
mapview(HUC_8_wind, zcol = "wind")
  

#view with ggplot
ggplot()+
  geom_sf(data = HUC_8_wind, aes(fill(wind)))
...
intersect_p2 <- huc8_sf %>%  
  st_filter(triCo)


#Intersect the HUC_8s
mapview(intersect_p2, alpha.regions = 0, color='red')
