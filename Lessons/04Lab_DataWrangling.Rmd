...
#%in% --> keep values with the following names in the selected column
...
NTL.phys.data.year <- filter(NTL.phys.data, year4 %in% c(1999))

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesyear <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake") & year4 %in% c(1990:1999))

NTL.phys.data.Tuesyear2 <- NTL.phys.data %>% 
...
meansurftemp <- NTL.phys.data.tuesjuly %>% 
  filter(depth == 0) %>% 
