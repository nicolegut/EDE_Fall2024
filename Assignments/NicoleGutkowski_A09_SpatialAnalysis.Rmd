...
> ANSWER: 
> The datum that this CRS is associated with is the World Geodetic System 1984 ensemble. This is a projected coordinate reference system. The EPSG code for the counties dataset is 4326. 
...
gage_loc_csv <- read.csv(here('Data/Raw/NWIS_SiteInfo_NE_RAW.csv'),
...
glimpse(gage_loc_csv)

```

9.    What columns in the dataset contain the x and y coordinate values, respectively?  
> ANSWER:The columns that contain the x and y values are dec_long_va and dec_lat_va, respectively.
> 
...
gage_loc_sf <- st_as_sf(gage_loc_csv, 
...
>ANSWER: A new geometry field appeared in the shapefile dataframe with matched latitude, longitude coordinate pairs. The separate dec_long_va and dec_lat_va fields disappeared. 
>
...
NE_gage_county <- ggplot()+
  geom_sf(data = counties_sf)+
  geom_sf(data = gage_loc_sf)
NE_gage_county
...
site_flow_csv <- read.csv(here('Data/Raw/NWIS_SiteFlowData_NE_RAW.csv'),
  colClasses = c('site_no' = 'factor',
                 'station_nm' = 'factor'))


#15. Show the column names
colnames(site_flow_csv)


#16. Join the flow data to our NWIS gage location spatial dataframe
flow_gage_join <- gage_loc_sf %>% 
  inner_join(
    site_flow_csv, 
...
gage_height_map <- ggplot() +
  geom_sf(data = counties_sf, fill = "grey") +
  geom_sf(data = flow_gage_join, aes(size = gage_ht), 
...
flow_gage_join_utm <- st_transform(flow_gage_join, crs = 32614)


#21 Plot the data
utm_map <- mapview(counties_sf_utm, 
                   col.regions = "seagreen", 
                   alpha.regions = 1)+
  mapview(flow_gage_join_utm, 
...
lancaster_gages <- flow_gage_join_utm %>%
...
gages_within_15km <- flow_gage_join_utm %>%
  st_filter(lancaster_buffer, .predicate = st_intersects)

#25 Plot
lancaster_map <- ggplot() +
  geom_sf(data = counties_sf_utm, fill = "lightgrey")+
  geom_sf(data = lancaster_county, color = "yellow", lwd = 1)+
  geom_sf(data = gages_within_15km, color = "brown2")+
  geom_sf(data = lancaster_gages, color = "blue")
