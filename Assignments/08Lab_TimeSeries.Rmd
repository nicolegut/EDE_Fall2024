...
BR_Rainfall_ts <- ts(BR_complete$Rainfall, start = c(f_year, f_month), frequency = 12)
...
BR.rain.decomp <- stl(BR_Rainfall_ts, s.window = "periodic")

plot(BR.rain.decomp)
#bars on the side are magnitude of the variable relative to the trend of our data set. Larger bar = less component strength

BR.temp.decomp <- stl(BR_Temp_ts, s.window = "periodic")
...
BR.rain.components <- as.data.frame(BR.rain.decomp$time.series[,1:3])

BR.rain.components <- mutate(BR.rain.components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)

BR.temp.components <- as.data.frame(BR.temp.decomp$time.series[,1:3])

BR.temp.components <- mutate(BR.temp.components,
...
ggplot(BR.temp.components) +
  geom_line(aes(y = Observed, x = Date)) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  ylab(expression("Temperature (C"^o*")"))+
